...
function afficherTva() {
  ...
  if (tbDed) {
    ...
    else rows2 += '<tr style="background:rgba(230,57,70,.08);font-weight:700"><td>TOTAL</td><td></td><td></td><td></td><td style="color:var(--accent)">' + euros(summary.totalDeductible) + '</td><td></td><td></td></tr>';
  ...
  if (soldeEl) {
    ...
    soldeEl.innerHTML = '<div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px;text-align:center">'
      ...
      + '<div style="padding:16px;background:rgba(230,57,70,.06);border-radius:10px;border:1px solid rgba(230,57,70,.2)"><div style="font-size:.75rem;color:var(--text-muted);margin-bottom:6px">TVA Déductible</div><div style="font-size:1.3rem;font-weight:800;color:var(--accent)">' + euros(summary.totalDeductible) + '</div></div>'
