...
(function () {
  ...
  // Hotfix R1 (2026-05-09) — restauration OCR supprimee par PR #34.
  ...
  const TEMPLATE = `
    ...
    <div id="ai-chat-panel" role="dialog" aria-label="Assistant IA Gemini" aria-hidden="true">
      <header class="ai-chat-header">
        ...
        <button id="ai-chat-attach-btn" class="ai-chat-iconbtn" type="button" title="Scanner une facture / ticket / RIB" aria-label="Scanner un document">📎</button>
  ...
  const STYLES = `
    ...
    #ai-chat-fab {
      ...
      height: 56px;
      border-radius: 50%;
      background: linear-gradient(135deg, #e63946 0%, #c0303a 100%);
      color: #fff;
      border: none;
      cursor: pointer;
  ...
  function wireEvents() {
    ...
    if (attachBtn) attachBtn.addEventListener('click', openOcrSheet);
    const ocrCloseBtn = document.getElementById('ai-chat-ocr-close');
    if (ocrCloseBtn) ocrCloseBtn.addEventListener('click', closeOcrSheet);
    const ocrBackdrop = document.querySelector('#ai-chat-ocr-sheet .ai-chat-ocr-backdrop');
    if (ocrBackdrop) ocrBackdrop.addEventListener('click', closeOcrSheet);
    const ocrFile = document.getElementById('ai-chat-ocr-file');
    if (ocrFile) ocrFile.addEventListener('change', onOcrFileSelected);
    ...
    state.history.forEach((m, idx) => {
      ...
      if (m.role === 'user') {
        ...
        if (m._tools && m._tools.length) {
          ...
          tools.textContent = '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.8-3.8a6 6 0 0 1-7.9 7.9l-6.9 6.9a2.1 2.1 0 0 1-3-3l6.9-6.9a6 6 0 0 1 7.9-7.9l-3.8 3.8z"/></svg> ' + m._tools.join(', ');
  ...
  // ----- OCR (📎 scanner facture / ticket / RIB) — Hotfix R1 (2026-05-09) -----
  //
  // Code restaure depuis la branche claude/ai-ocr-multimodal (commit 4d2dc19),
  // supprime par erreur dans la PR #34. Edge fn ai-ocr v3 reste deployee et
  // active en prod, donc ce front se reconnecte direct sans changement backend.
  ...
  function renderOcrModeSelection() {
    ...
    hint.textContent = 'Photo nette + horizontale = meilleur resultat. Compression auto avant envoi. PDF accepte.';
  ...
  // Skip pour les PDF (pas compressables, envoyes tels quels).
  ...
  async function onOcrFileSelected(e) {
    ...
    const isPdf = /pdf/i.test(file.type || '') || /\.pdf$/i.test(file.name || '');
    if (!isImg && !isPdf) {
      renderOcrError('Le fichier choisi n\'est ni une image ni un PDF.', 'Selectionne une photo (JPG, PNG, HEIC) ou un PDF.');
      return;
    }
  ...
  function renderOcrResult(result) {
    ...
    fillBtn.addEventListener('click', () => prefillFormFromOcr(mode, data));
  ...
  function prefillFormFromOcr(mode, data) {
    ...
    try {
      ...
      console.warn('[ai-chat OCR] prefillFormFromOcr error', err);
  ...
  window.AIChat = {
    ...
    clear: clearHistory,
    state: () => ({ ...state }),
    // OCR helpers exposes pour interop / tests / declenchement programmatique.
    openOcr: openOcrSheet,
    closeOcr: closeOcrSheet,
  };
})();
