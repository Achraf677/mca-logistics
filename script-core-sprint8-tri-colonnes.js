  ...
  if (filtreStatut) livraisons = livraisons.filter(l => filtreStatut === 'brouillon'
    ...
    : l.statut === filtreStatut);
      ...
      tb.innerHTML = paged.slice.map(l => {
        ...
        const statutBrouillonSelS8 = (l.statut === 'en-attente' || l.statut === 'brouillon' || !l.statut) ? 'selected' : '';
        const selectStatutPropre = '<select class="livraison-inline-select ' + getLivraisonInlineSelectClass('statut', l.statut) + '" onchange="changerStatutLivraison(\'' + l.id + '\',this.value,this);styliserSelectLivraison(this,\'statut\')"><option value="en-attente" ' + statutBrouillonSelS8 + '>Brouillon</option><option value="en-cours" ' + (l.statut === 'en-cours' ? 'selected' : '') + '>En cours</option><option value="livre" ' + (l.statut === 'livre' ? 'selected' : '') + '>Livré</option></select>';
